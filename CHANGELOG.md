...
- **song** `<title>` - Download high-quality MP3 from YouTube (128kbps)
- **ytmp3** `<link>` - Convert YouTube video to MP3 format
- **ytmp4** `<link>` - Download YouTube video as MP4 (max 5 min)
- **pause** - Pause the currently playing music
- **resume** - Resume paused music playback
- **stop** - Stop playback and clear the entire queue
- **nowplaying** / **np** - Display current track with progress bar, volume, effects

**Queue Management Commands:**
- **queue** `[page]` - View the music queue with pagination
- **addqueue** `<title/link>` - Add a song to the end of queue
- **remove** `<number>` - Remove a specific song from queue by position
- **clearqueue** - Clear all songs from the queue
- **playnext** `<title>` - Add a song to play next (front of queue)
- **skip** / **next** - Skip to the next song in queue
- **shuffle** - Randomize the order of songs in queue

**Loop & Autoplay:**
- **loop** - Toggle repeat for current track (🔂)
- **loopqueue** - Toggle repeat for entire queue (🔁)
- **autoplay** `[on/off]` - Toggle auto-suggested songs when queue ends

**Audio Effects (FFmpeg-powered):**
- **volume** `<0-200>` - Adjust playback volume (0-200%, supports boost)
- **bassboost** / **bass** `<0-20>` - Enhance bass levels (0=off, 20=extreme)
- **pitch** `<0.5-2.0>` - Change pitch (0.5=deep, 2.0=high)
- **speed** `<0.5-2.0>` - Change playback speed

**Search & Information:**
- **ytsearch** `<query>` - Search YouTube and show top 5 results
- **searchmusic** `<query>` - Search for songs (10 results)
- **songinfo** `<title/link>` - Get detailed song information
- **lyrics** `<title>` - Fetch song lyrics (auto-detects current playing)
- **history** `[limit]` - View recently played songs in this chat

**Audio Editing (Advanced):**
- **seek** `<seconds>` - Jump to specific time in track
- **trimmp3** `<link> <start> <end>` - Cut a portion of audio (times in seconds)
- **mergeaudio** `<link1> <link2>` - Combine two audio files into one
- **convertmp3** `<link>` - Convert any YouTube video to MP3

### Technical Implementation

**Database Integration:**
- Music queue persisted in MongoDB per group chat
- User music history stored in database
- Settings saved per-thread (volume, effects, loop status)

**Redis Caching:**
- Music state cached with 24-hour TTL
- Fast queue operations with Redis
- Automatic state sync between memory and Redis

**Audio Processing:**
- FFmpeg for all audio effects and conversions
- Real-time bass boost, pitch shift, and speed control
- Trim and merge capabilities
- Max duration: 10 minutes per track
- Max queue size: 50 songs

**Messenger Compatibility:**
- Audio sent as voice messages (attachment)
- Works in all Messenger Group Chats
- No streaming required - downloads and sends complete files
